...
while True:
    ...
    # .strip() lagane se agar space dab bhi gaya toh wo hat jayega
    choice = input("Enter your choice (1-5): ").strip()

    # 1 Add Expense
    if choice == "1":
        date = input("Enter date (DD-MM-YYYY): ")
        category = input("Enter category (Food, Travel, etc): ").strip().capitalize()
        description = input("Enter description: ")
        
        try:
            amount = float(input("Enter amount: ₹"))
        except ValueError:
            print("\n❌ Galat amount! Kripya sirf numbers daalein.")
            continue
